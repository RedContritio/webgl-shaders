...
if __name__ == "__main__":
    ...
    while not glfw.window_should_close(window):
        ...
        #     f.write(outBuffer)
        # outImage = Image.open(io.BytesIO(outBuffer))
        # outImage = np.frombuffer(outBuffer, dtype=np.uint8)
        # outImage = outImage.reshape(WINDOW_WIDTH, WINDOW_HEIGHT, 4)
        # outImage = Image.fromarray(outImage, 'RGBA')
        outImage = Image.frombytes('RGBA', (WINDOW_WIDTH, WINDOW_HEIGHT), outBuffer)
        outImage = outImage.transpose(Image.FLIP_TOP_BOTTOM)
        # outImage = ImageOps.flip(outImage)
